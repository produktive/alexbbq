...
| `REVERB_SERVER_*` | Server-side Reverb process (Pi default: `127.0.0.1:8081` over HTTP) |
...
`REVERB_SERVER_PORT` must match the port in `deploy/caddy/snippets/reverb.caddy` (default `8081`).
