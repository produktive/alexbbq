...
`REVERB_APP_*` credentials are generated by `php artisan reverb:configure` (included in the deploy steps above). Regenerate with `--force` if rotating keys. `VAPID_*` keys are generated by `php artisan webpush:configure` the same way.
